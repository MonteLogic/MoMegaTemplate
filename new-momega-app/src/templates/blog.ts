...
export async function generateBlogFiles(config: AppConfig) {
  if (!config.features.blog) return;

  const { projectPath } = config;

  // Create blog content directory
  await fs.ensureDir(path.join(projectPath, 'content', 'posts'));
...
Happy writing! 🎉`;
  ...
  await fs.writeFile(path.join(projectPath, 'content', 'posts', 'welcome.md'), examplePost);
...
export function getPostBySlug(slug: string): Post | null {
  ...
  await fs.writeFile(path.join(projectPath, 'lib', 'blog.ts'), blogUtilsContent);
...
export default function BlogPage() {
  ...
  await fs.writeFile(path.join(projectPath, 'app', 'blog', 'page.tsx'), blogPageContent);
...
export default function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  await fs.ensureDir(path.join(projectPath, 'app', 'blog', '[slug]'));
  await fs.writeFile(path.join(projectPath, 'app', 'blog', '[slug]', 'page.tsx'), blogPostPageContent);
}
