...
export async function generatePWAFiles(config: AppConfig) {
  ...
  await fs.writeJson(path.join(projectPath, 'public', 'manifest.json'), manifestContent, { spaces: 2 });
...
function updateCache(request, response) {
  ...
  await fs.writeFile(path.join(projectPath, 'public', 'sw.js'), serviceWorkerContent);
...
<body>
  ...
  await fs.writeFile(path.join(projectPath, 'public', 'offline.html'), offlinePageContent);
...
export function PWAProvider({ children }: { children: React.ReactNode }) {
  ...
  await fs.writeFile(path.join(projectPath, 'components', 'pwa-provider.tsx'), pwaComponentContent);
}
