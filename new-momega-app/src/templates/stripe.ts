...
export async function generateStripeFiles(config: AppConfig) {
  if (!config.features.stripe) return;

  const { projectPath } = config;

  // Generate Stripe API routes
  await fs.ensureDir(path.join(projectPath, 'app', 'api', 'stripe'));
...
export async function POST(request: NextRequest) {
  ...
  await fs.writeFile(path.join(projectPath, 'app', 'api', 'stripe', 'checkout', 'route.ts'), checkoutRouteContent);
...
export async function POST(request: NextRequest) {
  ...
  await fs.writeFile(path.join(projectPath, 'app', 'api', 'stripe', 'webhook', 'route.ts'), webhookRouteContent);
...
export default function PricingPage() {
  ...
  await fs.writeFile(path.join(projectPath, 'app', 'pricing', 'page.tsx'), pricingPageContent);
...
export async function cancelSubscription(subscriptionId: string) {
  ...
  await fs.writeFile(path.join(projectPath, 'lib', 'stripe.ts'), stripeUtilsContent);
}
