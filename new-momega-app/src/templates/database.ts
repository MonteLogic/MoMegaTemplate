...
export async function generateDatabaseFiles(config: AppConfig) {
  ...
  await fs.ensureDir(path.join(projectPath, 'db', 'migrations'));
...
export const posts = sqliteTable('posts', {
  ...
  await fs.writeFile(path.join(projectPath, 'db', 'schema.ts'), schemaContent);
...
export const db = drizzle(sqlite, { schema });`;
  ...
  await fs.writeFile(path.join(projectPath, 'db', 'index.ts'), dbIndexContent);
...
export async function POST(request: NextRequest) {
  ...
  await fs.ensureDir(path.join(projectPath, 'app', 'api', 'users'));
  await fs.writeFile(path.join(projectPath, 'app', 'api', 'users', 'route.ts'), apiRouteContent);
}
